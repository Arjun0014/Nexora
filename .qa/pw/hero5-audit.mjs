// Every stop of the hero on every profile: the active copy must be fully on screen and never overlap the window.
...
for (const profile of profiles) {
  const { browser, page } = await open(profile, { url: 'http://localhost:4321/?nointro&qa', wait: 800 });
  ...
  for (let stop = 0; stop <= 5; stop++) {
    ...
    const r = await page.evaluate(() => {
      ...
      const arch = { l: px('--arch-left'), t: px('--arch-top'), b: px('--arch-bottom'), w: px('--arch-w') };
      ...
      if (!portrait && box.r > arch.l - 12) issues.push(`overlaps the window by ${Math.round(box.r - arch.l + 12)}px`);
      if (portrait && box.t < arch.b + 4) issues.push(`under the window by ${Math.round(arch.b + 4 - box.t)}px`);
