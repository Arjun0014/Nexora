...
| `scenes/held.ts` | `charge` 0→1 in 2.9 s held, back in 1.4 s; `playbackRate = 1 + 6·charge²`; ground runs day→dusk→night between 0.12 and 0.42; lines at 0.46…0.86 with hysteresis; answer at 0.97 and latched | pointer, touch (pointer capture, no context menu) and Space/Enter on the button. Uses `loop-960.mp4`, loaded only near the scene. Sets `--hold` on `<html>` for the pointer's ring. |
